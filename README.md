...
## TADs Utilizados
1. **Time**
O TAD Time representa um time individual por meio de uma struct com os seguintes campos:
...
2. **BDTimes**
O TAD BDTimes representa a coleção de times no campeonato por meio de uma struct com os seguintes campos:
+ `times[10] (Time)`: Coleção dos 10 times no campeonato.

3. **Partida**
O TAD Partida representa uma partida unica por meio de uma struct com os seguintes campos:
...
4. **BDPartida**
O TAD BDPartida representa a coleção de partidas por meio de uma estrutura de vetor dinámico, tem os seguintes campos:
+ `Partidas (Partida*)`: Vetor de structs de partidas.
+ `num (int)`: Quantidade de partidas dentro do vetor dinámico.
+ `cap (int)`: Capacidade alocada de partidas do vetor dinámico.
...
+ Modularização do codigo: Funções especificas para a busca, leitura e armazenamento de dados de arquivos, e impressão de dados.
